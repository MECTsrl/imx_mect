...
MECT_BUILD_IMXMECT_BRANCH := mect_suite_3.5
...
MECT_DEFAULT_IMAGE := TPAC1007_04_AD
...
MECT_COMMON_RFSPKGS := \
	...
	qt-embedded-rfs-$(MECT_QT_VERSION)-1.$(MECT_TARGET_ARCH).rpm \
...
MECT_LTIB_QT_ARCH = qt-everywhere-opensource-src-$(MECT_QT_VERSION).tar.gz
MECT_LTIB_QT_PATCH1 = qt-everywhere-opensource-src-$(MECT_QT_VERSION)-1394522957.patch
MECT_LTIB_QT_PATCH2 = qt-everywhere-opensource-src-$(MECT_QT_VERSION)-1420823826.patch
MECT_LTIB_QT_PATCH3 = qt-everywhere-opensource-src-$(MECT_QT_VERSION)-1420823825.patch

MECT_LTIB_OPENSSL_ARCH = openssl-$(MECT_OPENSSL_VERSION).tar.gz
MECT_LTIB_OPENSSL_PATCH1 = openssl-$(MECT_OPENSSL_VERSION)-fix_parallel_build.patch
...
.PHONY: all
all: env downloads setup build image target_dev sysupdate_mrs sysupdate_hardening sysupdate_passwordreset sysupdate_netreset sysupdate_calreset sysupdate_micropython sysupdate_mqtt sysupdate_factory
...
ltib_patch: downloads
	...
	cp -a --reflink=auto $(MECT_LTIBDIR)/config $(MECT_LTIBDIR)/dist $(MECT_LTIBDIR)/bin $(MECT_LTIBDIR)/ltib $(MECT_LTIBDIR_REF)
...
ltib_update:
	...
	cp -a --reflink=auto $(MECT_LTIBDIR_REF) $(MECT_LTIBDIR_PATCH)
