...
#QT working conifguration with qtquick
#
#./configure -embedded arm -xplatform qws/linux-arm-gnueabi-g++ -platform linux-g++ -release -static -opensource -confirm-license \
# -qt-gfx-linuxfb -qt-kbd-tty -qt-kbd-linuxinput -qt-mouse-tslib -host-little-endian -little-endian \
# -no-webkit -no-largefile -no-svg -no-qt3support -no-cups -no-nis \
# -no-sql-sqlite -no-audio-backend -no-gfx-multiscreen \
# -nomake demos -nomake examples -nomake translations \
#-no-scripttools -no-javascript-jit -no-openssl -no-stl -qt-zlib -qt-libpng -qt-libjpeg -qt-libtiff -no-neon -no-opengl 2> imx_configure_error
#
#
...
./configure --prefix=$RPM_BUILD_DIR/../../rootfs/usr/local/Trolltech -embedded arm -xplatform qws/linux-g++-mx -release -fast -shared  -no-largefile \
 -qt-gfx-transformed -qt-gfx-linuxfb -qt-gfx-vnc -qt-kbd-tty -declarative -declarative-debug -no-accessibility -no-stl -no-xmlpatterns -no-qt3support -no-multimedia -no-phonon -no-phonon-backend -no-svg -no-webkit -no-javascript-jit -script -scripttools -no-cups -no-nis -no-mmx -no-3dnow -no-sse -no-sse2 \
 -qt-mouse-tslib -plugin-sql-sqlite -little-endian -host-little-endian -fontconfig -sm -v -opensource -confirm-license -make demos -make examples  $XTRA_OPTS
