...
./configure --prefix=$RPM_BUILD_DIR/../../rootfs/usr/local/Trolltech -embedded arm -xplatform qws/linux-g++-mx -release -fast -shared  -no-largefile \
 -qt-gfx-transformed -qt-gfx-linuxfb -qt-gfx-vnc -qt-kbd-tty -no-accessibility -no-stl -no-xmlpatterns -no-qt3support -no-multimedia -no-phonon -no-phonon-backend -no-svg -no-webkit -no-javascript-jit -no-script -no-scripttools -no-declarative -no-mmx -no-3dnow -no-sse -no-sse2 \
 -qt-mouse-tslib -plugin-sql-sqlite -little-endian -host-little-endian -fontconfig -sm -v -opensource -confirm-license -nomake demos -nomake examples  $XTRA_OPTS
