...
sub release_copy
{
    my ($url, $ddir, $upps) = @_;
    my ($file) = $url =~ m-/?([^/]+)$-;
    return 1 if -f "$ddir/$file";

    my ($ret, $sav);
    my @sav_list = qw/pp_list force_md5get wget_opts quiet/;
    foreach my $k (@sav_list) { $sav->{$k} = $cf->{$k} }
    my $pps = $upps || "gpp_stage cpp_stage";
    $cf->{pp_list}      = [ split(/\s+/, $pps) ];
    $cf->{force_md5get} = 1;
    $cf->{quiet}        = 1;
    $cf->{wget_opts}   .= " -q";
...
sub release_copy_pkg
{
    ...
    foreach my $url (  split(/\s*\n/, $tok->{sources}),
        ...
        release_copy($url, $ddir) or return;
    }
...
{
    my ($fullbsp, $distmap, $pkgs, $logfile) = @_;
    my $msg;
...
TXT
    ...
    release_copy($rpm, $pkgs)  or die "Error: $rpm  required by defconfig\n";
    release_copy($srpm, $pkgs) or die "Error: $srpm required by defconfig\n";
    ...
    return 1 unless ($fullbsp || $distmap);
...
TXT
    ...
    release_copy_content($args->{fullbsp}, $args->{distmap}, 'pkgs', $logfile);
