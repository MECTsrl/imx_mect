...
# add indirect dependencies (module-init-tools needs this, busybox doesn't)
foreach my $module ( keys %$mod ) {
    foreach my $dep ( keys %{$mod->{$module}} ) {
        foreach my $subdep ( keys %{$mod->{$dep}} ) {
           $mod->{$module}{$subdep} = 1;       
        }
    }
}
...
# $Id: depmod.pl,v 1.7 2007/04/02 14:31:56 r61694 Exp $
