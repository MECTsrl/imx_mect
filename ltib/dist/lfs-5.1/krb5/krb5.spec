...
Patch1          : %{name}-%{version}-fix-EAI_NODATA-undefined-check.patch
Patch2          : %{name}-%{version}-fix-conflicting-types-for-getline.patch
...
./configure --prefix=%{_prefix} --host=$CFGHOST --build=%{_build} \
            --enable-shared  --without-tcl --with-netlib="$DEV_IMAGE/lib/libresolv-2.11.1.so"
make
