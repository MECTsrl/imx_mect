...
Patch20          : qtopia-opensource-4.3.0-gcc-3.4.6-build-fix.patch
...
case "$PLATFORM" in
	...
	imx31ads | imx32ads | imx27ads | mxc* )
	...
	imx35_3stack )
 	export QWS_PLATFORM=mxc
  	...
  	export EXTRA_QTE_CONFIG=" -qt-mouse-tslib -qt-kbd-usb"
  	#export EXTRA_QPE_CONFIG_HELIX=" -helix -helix-system-id linux-2.2-libc6-arm9-cross-gcc4 "
  	export EXTRA_QPE_CONFIG=" -displaysize 800x480 $Q_SOUND_CONFIG $Q_MEDIA_CONFIG $Q_IRDA_CONFIG -no-drm -no-bluetooth -no-modem -no-sxe -no-voip -no-vpn"
  	export QWS_KEYBOARD=USB:/dev/input/keyboard0
  	export QWS_MOUSE_PROTO=Tslib:/dev/input/ts0
  	export QWS_DISPLAY=LinuxFb:mmWidth=90:mmHeight=54
	...
	export EXTRA_QTE_CONFIG=" -qt-mouse-tslib -qt-kbd-usb"
	#export EXTRA_QPE_CONFIG_HELIX=" -helix -helix-system-id linux-2.2-libc6-arm9-cross-gcc4 "
	export EXTRA_QPE_CONFIG=" -displaysize 480x640 $Q_SOUND_CONFIG $Q_MEDIA_CONFIG $Q_IRDA_CONFIG -no-drm -no-bluetooth -no-modem -no-sxe -no-voip -no-vpn"
	export QWS_KEYBOARD=USB:/dev/input/keyboard0
	export QWS_MOUSE_PROTO=Tslib:/dev/input/ts0
	export QWS_DISPLAY=LinuxFb:mmWidth=54:mmHeight=72
...
if [ `uname -m` = "x86_64" ] ; then
	QT_HOST="-platform linux-g++-64"
fi
if [ "$EXTRA_QTE_CONFIG" = "" ] ; then
	echo yes | ./configure $QT_HOST -device $QWS_PLATFORM -verbose
else
	echo yes | ./configure $QT_HOST -device $QWS_PLATFORM -verbose -extra-qtopiacore-config "${EXTRA_QTE_CONFIG} -I ${DEV_IMAGE}/usr/include -L ${DEV_IMAGE}/usr/lib"
fi
