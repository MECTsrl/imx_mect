...
#ifdef CONFIG_XENO_DRIVERS_CAN_FLEXCAN
/* from linux-2.6.35.3_ipipe/drivers/net/can/flexcan/dev.c */
...
#ifdef CONFIG_XENO_DRIVERS_CAN_FLEXCAN
{
	...
	if (found) {
	    ...
	    bit_time->brp = div/i - 1;
	    bit_time->prop_seg = segment->propseg;
	    bit_time->phase_seg1 = segment->pseg1;
	    bit_time->phase_seg2 = segment->pseg2;
	    bit_time->sjw = 2;
	    bit_time->sam = 0;
	} else {
	    // default for iMX28 and 500kbit/s rate
	    bit_time->brp = 2;		// PRESDIV
	    bit_time->prop_seg = 4;	// PROPSEG
	    bit_time->phase_seg1 = 4;	// PSEG1
	    bit_time->phase_seg2 = 4;	// PSEG2
	    bit_time->sjw = 2;		// RJW
	    bit_time->sam = 0;		// dontcare
	}
}
#else
