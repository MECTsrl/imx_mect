...
#if (defined(CONFIG_FB_MXS_LCD_TM043) || defined(CONFIG_FB_MXS_LCD_HH057) || defined(CONFIG_FB_MXS_LCD_RK043) || defined(CONFIG_FB_MXS_LCD_HH057_640x480) ) && defined(CONFIG_ARCH_MX28)
...
static int lcdif_set_rate(struct clk *clk, unsigned long rate)
{
	...
	// 8Mhz
#if (defined(CONFIG_FB_MXS_LCD_TM043) || defined(CONFIG_FB_MXS_LCD_HH057)) && defined(CONFIG_ARCH_MX28)
	reg_val |= (3 << BP_CLKCTRL_DIS_LCDIF_DIV) & BM_CLKCTRL_DIS_LCDIF_DIV; // 8MHz
#elif defined(CONFIG_FB_MXS_LCD_RK043) && defined(CONFIG_ARCH_MX28)
	reg_val |= (2 << BP_CLKCTRL_DIS_LCDIF_DIV) & BM_CLKCTRL_DIS_LCDIF_DIV; //12Mhz
#elif (defined(CONFIG_FB_MXS_LCD_HH057_640x480)) && defined(CONFIG_ARCH_MX28)
	reg_val |= (1 << BP_CLKCTRL_DIS_LCDIF_DIV) & BM_CLKCTRL_DIS_LCDIF_DIV; //24Mhz
#else // 24Mhz
...
static int lcdif_set_rate(struct clk *clk, unsigned long rate)
{
	...
	printk(KERN_INFO "Programming PFD=%u,DIV=%u ref_pix=%uMHz "
...
static unsigned long lcdif_get_rate(struct clk *clk)
{
	...
	if (div) {
		rate /= div;
		div = (__raw_readl(CLKCTRL_BASE_ADDR + HW_CLKCTRL_FRAC1) &
			BM_CLKCTRL_FRAC1_PIXFRAC) >> BP_CLKCTRL_FRAC1_PIXFRAC;
		rate /= div;
	}
