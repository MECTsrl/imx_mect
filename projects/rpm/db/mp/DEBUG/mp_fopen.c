 ...
 * db-3.3.11 tries to do an off page hash access through a mempool, tries
 * to reopen the original database path.
 */
 ...
 */
int
__Xmemp_fopen(dbmp, mfp, path, flags, mode, pagesize, needlock, finfop, retp, f, l)
	DB_MPOOL *dbmp;
	MPOOLFILE *mfp;
	const char *path;
	u_int32_t flags;
...
fprintf(stderr, "__memp_fopen(%s): %s:%u\n", path, f, l);
	dbenv = dbmp->dbenv;
	ret = 0;
	rpath = NULL;
...
if (chroot_prefix) {
	...
	chrlen = 0;
		...
		if ((ret =
		   __os_open(dbenv, rpath+chrlen, oflags, mode, &dbmfp->fh)) != 0) {
			if (!LF_ISSET(DB_EXTENT))
				__db_err(dbenv,
				    "%s: %s", rpath+chrlen, db_strerror(ret));
			goto err;
		}
		 ...
		 */
		if ((ret = __os_ioinfo(dbenv, rpath+chrlen,
		    &dbmfp->fh, &mbytes, &bytes, NULL)) != 0) {
			__db_err(dbenv, "%s: %s", rpath+chrlen, db_strerror(ret));
			goto err;
		}
		...
		if (finfop->fileid == NULL) {
			if ((ret = __os_fileid(dbenv, rpath+chrlen, 0, idbuf)) != 0)
				goto err;
			finfop->fileid = idbuf;
		}
	}
...
#define	DB_MAXMMAPSIZE	(10 * 1024 * 1024)	/* 10 Mb. */
	...
	if (F_ISSET(mfp, MP_CAN_MMAP)) {
		...
		if (__os_mapfile(dbenv, rpath+chrlen,
