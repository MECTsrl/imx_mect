...
sed -i '/\bMECT_BUILD_IMXMECT_TAG\b/ s/\(MECT_BUILD_IMXMECT_TAG.*=\).*/\1 '$IMX_MECT'/' Makefile

echo "Setting MECT_BUILD_ATCMCRT_TAG to $ATCMCONTROLRUNTIMESYSTEM"
sed -i '/\bMECT_BUILD_ATCMCRT_TAG\b/ s/\(MECT_BUILD_ATCMCRT_TAG.*=\).*/\1 '$ATCMCONTROLRUNTIMESYSTEM'/' Makefile

echo "Setting MECT_BUILD_PLUGINSCRT_TAG to $MECT_PLUGINS"
sed -i '/\bMECT_BUILD_PLUGINSCRT_TAG\b/ s/\(MECT_BUILD_PLUGINSCRT_TAG.*=\).*/\1 '$MECT_PLUGINS'/' Makefile

echo "Setting MECT_BUILD_APPSCRT_TAG to $MECT_APPS"
sed -i '/\bMECT_BUILD_APPSCRT_TAG\b/ s/\(MECT_BUILD_APPSCRT_TAG.*=\).*/\1 '$MECT_APPS'/' Makefile
