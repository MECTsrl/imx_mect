...
expr "$RELEASE" : @@THIS_VERSION_MAJ_MIN@@ > /dev/null || expr "$RELEASE" : 3\\.1 > /dev/null  || expr "$RELEASE" : 3\\.0 > /dev/null || expr "$RELEASE" : 2\\.0 > /dev/null || do_exit "cannot update the installed version ${RELEASE}."
